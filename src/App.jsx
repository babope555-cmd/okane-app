...
const CATEGORIES = [
  ...
  { id: "health", label: "健康・美容", icon: "🌿", iconBg: "#e0f4e8", sub: ["医療・治療・整体", "スキンケア・サプリ", "ジム・フィットネス", "メイク・化粧品", "アロマ・お香・ディフューザー", "ネイル", "ヘアサロン", "その他"] },
...
{
  "talents": [
    { "title": "才能の名前（10字以内）", "futsuu": "本人が普通にやっていることを、才能として具体的に描写する（数字やピース数の引用はしない）（45字以内）", "kachi": "その『普通』を必要としている人を具体的に挙げ、どう役立つかを書く（60字以内）", "emoji": "絵文字1つ" },
...
talentsのルール：
・futsuuは「あなたは〜」で始め、本人の実際の行動を才能として具体的に描写する。【蓄積ピース数】等の数字やデータ根拠は引用しない（読み手が知りたいのは才能そのものの中身）
        ...
        <div style={{ zIndex: 1, width: "100%", maxWidth: 320, marginBottom: 24, display: "flex", flexDirection: "column", gap: 12 }}>
          {talents.map((t, i) => (
            <div key={i} style={{
              ...
              {t.futsuu || t.kachi ? (
                <>
                  {/* ① あなたが"普通"だと思っていること（才能そのもの） */}
                  <div style={{ fontSize: 13, color: "#c07fb0", fontWeight: 700, letterSpacing: 1, marginBottom: 3 }}>あなたが"普通"だと思っていること</div>
                  ...
                  {/* ③ あなたの"価値"を必要としている人 */}
                  <div style={{ fontSize: 13, color: "#4db8c8", fontWeight: 700, letterSpacing: 1, marginBottom: 3 }}>あなたの"価値"を必要としている人</div>
...
function EmotionSliderEnhanced({ value, onChange }) {
  ...
  // ドラッグ位置から直接値を計算（ネイティブのスライドが効きにくい端末向けの保険）
  ...
  return (
    ...
    >
      ...
      {/* 自前実装スライダー（track＋thumbのdiv、ポインターイベントで直接制御）
          ネイティブinput[type=range]のドラッグが端末によって不安定だったため、
          ブラウザ標準のスライド挙動に頼らず、指の位置から直接値を計算する方式に変更 */}
      <div
        ref={trackRef}
        onPointerDown={e => {
        ...
        style={{
          position: "relative", width: "100%", height: 16, borderRadius: 999,
          background: "#fce7f3", cursor: "pointer", touchAction: "none",
        }}
      >
        <div style={{
          position: "absolute", left: 0, top: 0, bottom: 0, width: `${localVal}%`,
          borderRadius: 999, background: col, pointerEvents: "none",
          transition: "background 0.3s",
        }} />
        <div style={{
          position: "absolute", top: "50%", left: `${localVal}%`,
          width: 36, height: 36, borderRadius: "50%", background: "white",
          border: `3px solid ${col}`, boxShadow: "0 2px 10px rgba(0,0,0,0.15)",
    ...
    return (
      <div style={{ padding: "20px 20px 100px" }}>
        ...
        {history.some(h => h.aiFallback) && (
          <div style={{ marginTop: 8, marginBottom: 20, padding: "14px 16px", borderRadius: 16, background: `${COLORS.goldPrimary}18`, border: `1px solid ${COLORS.goldPrimary}44` }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div>
            ...
            {(() => {
              const SUB_EMOJI = {
                ...
                "メイク・化粧品":"💄","アロマ・お香・ディフューザー":"🕯️","ネイル":"💅","ヘアサロン":"💇",
                      ...
                      <div key={sub}>
                        <button onClick={() => {
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                            ...
                            {sub.includes("（") ? (
                              <span style={{ lineHeight: 1.4, textAlign: "left", whiteSpace: "nowrap" }}>
                                {sub.split("（")[0]}
                                <span style={{ fontSize: 11 }}>（{sub.split("（")[1]}</span>
                              </span>
                            ) : (
        ...
        {inputStep === "loading" && aiLoading && (() => {
          ...
          const LoadingCountdown = () => {
            const START_DELAY_MS = 500; // 最初の「5」が出るまでの間
            const TEMPO_MS = 2000; // 数字が切り替わる間隔（調整しやすいよう変数化）
            const HOLD_AT_1_MS = 2300; // 「1」が現れて消えるまでの尺（他の数字と同じくポンと出てから、すっと消える）
            const NUM_COLORS = ["#ff6b9d","#ffd93d","#6bcb77","#4d96ff","#c77dff"];
            ...
            React.useEffect(() => {
              ...
              const id = setInterval(() => {
                setCount(c => {
                  if (c <= 1) {
                    clearInterval(id);
                    setTimeout(() => setGone(true), HOLD_AT_1_MS);
                    return 1;
                  }
                  return c - 1;
                });
            ...
            return (
              <div style={{
                ...
                <div key={count} style={{
                  ...
                  color: NUM_COLORS[count % NUM_COLORS.length],
                  opacity: 0.4,
                  animation: count === 1 ? `numberPopFade ${HOLD_AT_1_MS}ms both` : "bounceIn 1s both",
                }}>{count}</div>
              </div>
            );
          };
  ...
  return (
    <div onClickCapture={handleGlobalClick} style={{
      ...
      <style>{`
        ...
        @keyframes numberPopFade { 0%{transform:scale(0.3);opacity:0} 35%{transform:scale(1.15);opacity:1} 50%{transform:scale(1);opacity:1} 100%{transform:scale(1.3);opacity:0} }
      ...
      {showTalentMatch && <TalentMatchModal
        ...
        insightMessage={aiResult?.insightMessage || history.find(h => h.insightMessage)?.insightMessage || ""}
        topValues={aiResult?.topValues || (() => {
          // マイページから開いた場合はaiResultが無いので、履歴から同じロジックで再計算
          const tagCountMap = {};
          history.forEach(h => (h.valueTags || []).forEach(t => { tagCountMap[t] = (tagCountMap[t] || 0) + 1; }));
          return Object.entries(tagCountMap).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([t]) => t);
        })()}
        topTalents={aiResult?.topTalents || (() => {
          ...
          history.forEach(h => (h.valueTags || []).forEach(t => { tagCountMap[t] = (tagCountMap[t] || 0) + 1; }));
          const domainScore = {};
          Object.entries(tagCountMap).forEach(([tag, cnt]) => {
            const d = DOMAIN_MAP[tag];
            if (d) domainScore[d] = (domainScore[d] || 0) + cnt;
          });
          return Object.entries(domainScore).sort((a, b) => b[1] - a[1]).slice(0, 2).map(([d]) => d);
        })()}
        aiPct={(() => {
          const loveCount = history.filter(h => (h.aiPct ?? 0) >= 65).length;
