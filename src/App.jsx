        ...
        {!submitted ? (
          <>
            ...
            <button onClick={handleSubmit} disabled={submitting} style={{
              ...
              border: "none", color: "#3a2800", cursor: submitting ? "default" : "pointer",
              opacity: submitting ? 0.6 : 1,
              boxShadow: "0 4px 20px #c49a2a44", marginBottom: 10,
              zIndex: 1, position: "relative",
            }}>{submitting ? "送信中…" : "✦ 登録する"}</button>
            {error && (
              <div style={{ fontSize: 11, color: "#c0392b", marginBottom: 10, zIndex: 1, position: "relative" }}>
...
export default function App() {
  ...
  // タブ切り替え時は常にページ最上部から表示する（マイページ等の途中スクロール位置が残るバグ対策）
  ...
  const applyEnergyGain = (amount) => {
    ...
    if (newLevelObj.lv > level) {
      ...
      if (newLevelObj.lv === 5) setTimeout(() => setShowEmailRegister(true), 4000);
    ...
    return (
      <div style={{ padding: "20px 20px 100px" }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ marginBottom: 16, borderRadius: 16, border: `1px solid ${COLORS.circleBlue}55`, overflow: "hidden" }}>
              <button onClick={() => setFutureOpen(o => !o)} style={{
                width: "100%", display: "flex", alignItems: "center", gap: 8,
                padding: "14px 16px", background: `${COLORS.indigoDeep}88`,
                border: "none", cursor: "pointer", textAlign: "left",
              }}>
                <span style={{ fontSize: 16 }}>✨</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: COLORS.softWhite, flex: 1 }}>今後の展開</span>
                <span style={{ fontSize: 11, color: COLORS.mutedText }}>{futureOpen ? "▲" : "▼"} （クリックで開く）</span>
              </button>
              {futureOpen && (
                <div style={{ padding: "16px 16px 20px", background: `${COLORS.indigoDeep}55` }}>
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 12, color: COLORS.goldLight, fontWeight: 700, marginBottom: 6 }}>
                      ・アプリがあなたと相性の良い人を自動で見つける
                    </div>
                    <div style={{ fontSize: 12, color: "#c090d8", fontWeight: 700 }}>
                      ・見つかったら、お互いに自動で通知が届く
                    </div>
                  </div>
                  <div style={{ fontSize: 11, color: COLORS.mutedText, lineHeight: 1.9, marginBottom: 14 }}>
                    時期はまだ確定していませんが、<br />
                    このようなヴァージョンアップを考えています。<br />
                    この機能が始まったらいち早く知りたい方は、<br />
                    メールアドレスをご登録ください。
                  </div>
                  <a href="https://forms.gle/BqQgXc4pnQ1d92RN7" target="_blank" rel="noopener noreferrer" style={{
                    display: "inline-block", padding: "10px 20px", borderRadius: 10, fontSize: 12, fontWeight: 700,
                    background: `${COLORS.circleBlue}33`, border: `1px solid ${COLORS.circleBlue}77`,
                    color: COLORS.aquaLight, cursor: "pointer", textDecoration: "none",
                  }}>✦ 登録する</a>
                </div>
              )}
            </div>
        ...
        {/* プレミアム機能（ライセンスキー） */}
        <div style={{ marginTop: 8, padding: "14px 16px", borderRadius: 16, background: `${COLORS.indigoDeep}44`, border: `1px solid ${COLORS.goldPrimary}55` }}>
