...
function AIResultCard({ result, onClose, onCloseToHome }) {
  ...
  const shakaiBonus = result.shakaiBonus || 0;
        ...
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          {shakaiBonus > 0 && (
            <div style={{
              ...
              borderRadius: 999, padding: "4px 14px", fontSize: 12, fontWeight: 700, color: "#0284c7",
            }}>
              🌍 社会貢献ボーナス +{shakaiBonus}pt
            </div>
          )}
        </div>
...
export default function App() {
  ...
  const applyEnergyGain = (amount) => {
    ...
    if (newLevelObj.lv > level) {
      ...
      // メール登録モーダル(ネクストステージへ)は表示しない設定に変更
...
補完ピースがある場合は、circulation_questとして小さな挑戦を1つ提案してください。`;
    ...
    try {
      ...
      const shakaiBonus = selCat?.label === "社会貢献・応援" ? 5 : 0;
      const emotionBonus = isBonus ? 8 : 0;
      const finalEnergy = baseEnergy + emotionBonus + comfortBonus + timeInvestBonus + jikoBonus + shakaiBonus;

      const result = {
        tags, energy: finalEnergy, bonus: isBonus, pending: isPending, story, jikoBonus, shakaiBonus,
      ...
      const fallbackEnergy = (count >= 3 ? 5 : count === 2 ? 4 : 3) + (fallbackIsBonus ? 8 : 0) + ((selTimeInvest || []).length > 0 ? 5 : 0) + (selCat?.label === "自己投資" ? 5 : 0) + (selCat?.label === "社会貢献・応援" ? 5 : 0);
      const fallbackEntry = {
        id: `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        date: new Date().toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" }),
      ...
      setAiResult({
        ...
        shakaiBonus: selCat?.label === "社会貢献・応援" ? 5 : 0,
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        ...
        {level >= 5 && (
          <div style={{ background: "linear-gradient(135deg, #1a0a3a, #2a0a5a)", borderRadius: 20, padding: 20, marginBottom: 16, border: "1px solid #c07fb066", textAlign: "center", boxShadow: "0 0 40px #c07fb033" }}>
            ...
            <div style={{ fontSize: 12, color: "#aaa", marginBottom: 16 }}>あなたの支出から導き出した循環パターンから「普通・あたりまえ」が価値・才能に変わる瞬間を発見します。<span style={{ fontSize: 10, color: "#8a7a9a", marginLeft: 4 }}>（プレミアム機能）</span></div>
            ...
            <div style={{ marginTop: 24 }}>
              ...
              <div style={{ marginTop: 16, background: `${COLORS.indigoDeep}cc`, borderRadius: 16, padding: 16, border: `1px solid ${COLORS.circleBlue}` }}>
                ...
                {[
                  ...
                  { label: "社会貢献ボーナス", val: "+5pt", desc: "カテゴリ「社会貢献・応援」を選ぶと加算" },
                ].map((r, i, arr) => (
