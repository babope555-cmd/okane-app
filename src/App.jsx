...
// ─── Future Updates Panel（今後の展開・メール登録／補完ピース図鑑カードの中に表示）────
function FutureUpdatesPanel() {
  ...
  return (
    <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid #ecd8f5" }}>
      <button onClick={() => setOpen(o => !o)} style={{
        width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between",
        background: "none", border: "none", cursor: "pointer", padding: 0,
      }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#7a5a98" }}>✨ 今後の展開</span>
        <span style={{ fontSize: 12, color: "#a090b0", display: "inline-block", transform: open ? "rotate(180deg)" : "none", transition: "transform 0.3s" }}>▼</span>
      </button>
      {open && (
        <div style={{ marginTop: 14, textAlign: "left", animation: "slideUpIn 0.4s cubic-bezier(0.16,1,0.3,1)" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#c49a2a", marginBottom: 8, lineHeight: 1.7, textAlign: "left" }}>
            ・アプリがあなたと相性の良い人を自動で見つける
          </div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#c07fb0", marginBottom: 12, lineHeight: 1.7, textAlign: "left" }}>
            ・見つかったら、お互いに自動で通知が届く
          </div>
          <div style={{ fontSize: 11, color: "#5a4a68", lineHeight: 1.8, marginBottom: 14, textAlign: "left" }}>
            時期は未定ですが、このような機能を検討しています。始まったらいち早く知りたい方は、メールをご登録ください。
          </div>
          {!submitted ? (
            <>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                ...
                style={{
                  ...
                  border: "1.5px solid #c07fb066", outline: "none",
                  background: "#faf7fd", color: "#3a2800", marginBottom: 10,
                  boxSizing: "border-box",
                }}
              />
              <button onClick={handleSubmit} disabled={submitting} style={{
                width: "100%", padding: 10, borderRadius: 999, fontSize: 12, fontWeight: 700,
              ...
              {error && (
                <div style={{ fontSize: 10, color: "#c07f7f", marginTop: 8, textAlign: "center" }}>
                  登録に失敗しました。時間をおいて再度お試しください。
                </div>
              )}
            </>
          ) : (
            <div style={{ fontSize: 12, color: "#7a5a98", textAlign: "center", padding: "8px 0" }}>
              ✓ 登録しました。ありがとうございます！
            </div>
          )}
        </div>
...
export default function App() {
  ...
  // タブが切り替わるたびにスクロール位置を最上部へリセット
  // （マイページ等を下までスクロールした状態から別タブへ移動し、また戻った際に
  //   途中の位置で表示されてしまう不具合の対策）
  useEffect(() => {
...
価値タグ: ${(entry.valueTags || []).join(", ") || "なし"}`;
        ...
        return (
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              ...
              {/* 右：循環レポートリンク */}
              <button onClick={() => { setTab("report"); window.scrollTo({ top: 0 }); }} style={{
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        ...
        {level >= 5 && (
          <div style={{ background: "linear-gradient(135deg, #1a0a3a, #2a0a5a)", borderRadius: 20, padding: 20, marginBottom: 16, border: "1px solid #c07fb066", textAlign: "center", boxShadow: "0 0 40px #c07fb033" }}>
            ...
            <div style={{ fontSize: 12, color: "#aaa", marginBottom: 16 }}>あなたの支出から導き出した循環パターンから「普通・あたりまえ」が価値・才能に変わる瞬間を発見します。</div>
        ...
        {history.some(h => h.aiFallback) && (
          <div style={{ marginTop: 8, marginBottom: 16, padding: "14px 16px", borderRadius: 16, background: `${COLORS.goldPrimary}18`, border: `1px solid ${COLORS.goldPrimary}44` }}>
        ...
        <div style={{ background: `${COLORS.indigoDeep}cc`, borderRadius: 20, padding: "18px 18px 14px", marginBottom: 16, border: `1px solid ${COLORS.circleBlue}` }}>
          <div style={{ fontSize: 11, color: COLORS.softWhite, letterSpacing: 1, marginBottom: 14 }}>循環要素の内訳</div>
  ...
  return (
    <div onClickCapture={handleGlobalClick} style={{
      ...
      {aiResult && <AIResultCard result={aiResult} onClose={() => { setAiResult(null); setShowInput(false); resetInput(); setTab("report"); window.scrollTo({ top: 0 }); }} onCloseToHome={() => { setAiResult(null); setShowInput(false); resetInput(); setTab("home"); window.scrollTo({ top: 0 }); }} />}
      {showSplash && <SplashScreen onDone={() => setShowSplash(false)} />}
      {/* utuwa.mp4: Lv5到達時演出（「レベル5に到達しました」メッセージを閉じた後にのみ再生） */}
      {level === 5 && !hasPlayedUtuwa && !pendingLevelUp && !levelUpInfo && (
